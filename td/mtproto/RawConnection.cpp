...
class RawConnectionDefault final : public RawConnection {
 public:
  ...
  size_t send_crypto(const Storer &storer, uint64 session_id, int64 salt, const AuthKey &auth_key,
    ...
    if (tls_pipeline_) {
      LOG(DEBUG) << "T3_CRYPTO: send_crypto wrote " << packet_size << " bytes to transport";
    }
    return packet_size;
  }
 ...
 private:
  ...
  Status flush_read(const AuthKey &auth_key, Callback &callback) {
    ...
    size_t socket_read_size = 0;
    if (r.is_ok()) {
      socket_read_size = r.ok();
      on_read(socket_read_size, callback);
    }
    // === TYPE3-PROXY BEGIN ===
    ...
    if (tls_pipeline_) {
      ...
      if (socket_read_size > 0) {
        LOG(DEBUG) << "T3_READ: socket received " << socket_read_size << " bytes, plaintext_avail=" << tls_pipeline_->plaintext_input()->size();
      }
    }
    // === TYPE3-PROXY END ===
    while (transport_->can_read()) {
      ...
      if (wait_size != 0) {
        ...
        if (tls_pipeline_) {
          LOG(DEBUG) << "T3_READ: transport needs " << wait_size << " more bytes";
        }
        break;
      }
      if (quick_ack != 0) {
  ...
  Status flush_write() {
    ...
    if (tls_pipeline_ && size > 0) {
      LOG(DEBUG) << "T3_WRITE: socket sent " << size << " bytes";
    }
    return Status::OK();
  }
